...
2. Downloads image and audio assets to local /tmp
3. Executes FFmpeg with Ken Burns effects
...
from ffmpeg_utils import build_ffmpeg_command, run_ffmpeg
...
def download_asset(url: str, asset_type: str, index: int) -> Optional[str]:
    """
    Download an asset (image or audio) from URL or GCS.
...
def main():
    ...
    audio_clips = [c for c in audio_clips if c.get('localFile')]
