...
// 2. Add skipDbUpdate to Narration fetch
code = code.replace(
...
// 3. Populate cleanedScenes for narration instead of just incrementing the count
code = code.replace(
...
// 5. Replace Music atomic update with in-memory modification
code = code.replace(
...
// 6. Replace SFX atomic update with in-memory modification
code = code.replace(
  /\/\/ ATOMIC UPDATE: Reload fresh data and update only sfxAudio field[\s\S]*?catch \(error: any\) \{[\s\S]*?console\.error\('\[Batch Audio\] Failed to update database for SFX:', error\)[\s\S]*?\}/,
  `cleanedScenes[i].sfxAudio = cleanedScenes[i].sfxAudio || {};
                  cleanedScenes[i].sfxAudio[\`\${i}-\${j}\`] = sfxUrl;`
);

// 7. Add FINAL DB UPDATE
code = code.replace(
