...
describe('the route composes a beat frame from its direction', () => {
  ...
  it('stamps stable prop tokens before compose so the unnamed-prop drop can match them', () => {
    ...
    expect(routeSrc).toContain('preserveLibraryPromptTokens: useInterleavedProRefs')
    expect(routeSrc).toContain('buildInterleavedReferencePairCaptions')
    expect(routeSrc).toContain('omitReferencesSection: useInterleavedProRefs')
