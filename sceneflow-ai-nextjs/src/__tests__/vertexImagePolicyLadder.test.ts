...
describe('nested 429 retry de-amplification', () => {
  it('scene generate-image skips outer burst after identity-ref exhaustion', () => {
    ...
    expect(src).toContain('signal: req.signal')
    expect(src).toContain('abortedByClient')
    expect(src).toContain('resolveVerifiedBeatRefsForApi')
    expect(src).toContain('Beat frame hydrated')
...
describe('express beat likeness gate', () => {
  ...
  it('passes the Express run lookbook through generateSceneImage', () => {
    ...
    expect(wrapper).toContain('combineAbortSignals')
    expect(wrapper).toContain('parentSignal')
    expect(wrapper).toContain('FRAME_AGENT_CANCELLED_MESSAGE')
  })
})
