...
describe('hero section copy and UI', () => {
  ...
  it('renders value chips without the pipeline strip in HeroSection', () => {
    ...
    expect(hero).toContain('top-16 z-20')
    expect(hero).toContain('lg:top-20')
    expect(hero).not.toContain('inset-x-0 bottom-0 z-20')
  ...
  it('extracts a dual-source background player and keeps theater on public files', () => {
    ...
    expect(background).toContain('object-cover')
    expect(background).toContain('object-top')
    expect(background).toContain('lg:object-contain')
