...
export const guidanceContent: Record<WorkflowStep, {
  ...
  dialogueAction: {
    ...
    tip: 'One strip runs Direction, Shots, Music, References, Pre-Vis, Video, Mixer, and Streams.',
  },
  directorsChair: {
    title: "Direction (in Writer's Room)",
    icon: <Film className="w-5 h-5 text-sf-primary" />,
    goal: 'Define cinematic direction that feeds pre-vis and Shot Frame generation.',
  ...
  storyboardPreViz: {
    ...
    goal: 'Build pre-vis frames for every shot and share for approval.',
    whyItMatters: 'Pre-vis frames are still images per shot — fast to change. Video and Shot Frames come later in Motion.',
    howItWorks: [
    ...
    toolsAndTips: [
      'Pre-vis Frame = still image for a shot (agent output)',
      'Approve pre-vis before opening Motion video work',
      'Gallery is the single pre-vis truth — video production lives in Motion',
    ],
    bestPractice: 'Share pre-vis for review before investing in Shot Frames and video.',
    tip: 'Making pre-vis changes is fast; re-rendering MP4 streams is slow.',
  },
  segmentBuilder: {
    title: 'Shots (internal)',
    icon: <Layers className="w-5 h-5 text-sf-primary" />,
    goal: 'Understand how shots drive cuts — managed automatically in the shot-first pipeline.',
    whyItMatters: 'Shots are script units (dialogue, narration, action). Agents and production derive pre-vis frames and shot clips from them.',
    howItWorks: [
      'Shots are created from your script automatically',
      'Pre-vis frames attach to shots via Stills Agent',
      'Motion derives shot clips for Shot Frames and video generation',
    ],
    toolsAndTips: [
      'Shot = script unit; Shot Frame = start/end pair for F2V',
      'Shots is a step on the same strip as Pre-Vis and Video',
    ],
    bestPractice: "Edit the script in the Writer's Room if shot boundaries need to change at the source.",
  ...
  callAction: {
    ...
    goal: 'Shot Frames → Mixer → Production Streams → Final Cut.',
    whyItMatters: 'This is canonical video production: Shot Frames for F2V, Director Console for shot video, Mixer for preview/export, Streams for finished MP4s.',
    howItWorks: [
    ...
    bestPractice: 'Complete Shot Frames before switching Output to Video. Gate video until start + end frames exist.',
