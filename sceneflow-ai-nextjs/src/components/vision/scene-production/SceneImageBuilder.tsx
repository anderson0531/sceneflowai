...
interface SceneImageBuilderProps {
  open: boolean
  onClose: () => void
...
export function SceneImageBuilder({
  open,
  onClose,
  segment,
  references,
  onGenerate,
  isGenerating = false
}: SceneImageBuilderProps) {
  const [mode, setMode] = useState<'guided' | 'advanced'>('guided')
  const [localIsGenerating, setLocalIsGenerating] = useState(false)
  const [structure, setStructure] = useState<SceneImageStructure>({
    location: '',
    timeOfDay: 'day',
    weather: 'clear',
    atmosphere: 'neutral',
  ...
  useEffect(() => {
    ...
    const updates: Partial<SceneImageStructure> = {}
      ...
      <DialogContent className="max-w-4xl h-[85vh] bg-gray-900 text-white border-gray-700 flex flex-col overflow-hidden">
        <DialogHeader className="flex-shrink-0">
          <DialogTitle className="text-white flex items-center gap-2">
            ...
            Scene Image Builder - Segment {segment?.sequenceIndex !== undefined ? segment.sequenceIndex + 1 : ''}
          </DialogTitle>
        </DialogHeader>
          ...
          <TabsContent value="advanced" className="space-y-4 mt-4">
            <div>
              <div className="flex items-center justify-between mb-2">
                <label className="text-xs text-gray-400">Custom Image Prompt</label>
              </div>
              <Textarea
        ...
        {isActuallyGenerating && (
          <div className="absolute inset-0 bg-black/70 backdrop-blur-md z-50 flex items-center justify-center rounded-lg">
            <div className="bg-gray-900 border-2 border-purple-500 rounded-xl p-8 shadow-2xl flex flex-col items-center max-w-sm">
              ...
              <h3 className="text-xl font-bold text-white mb-2">Generating Image</h3>
              <p className="text-sm text-gray-300 text-center">
                Creating your image visualization...
              </p>
