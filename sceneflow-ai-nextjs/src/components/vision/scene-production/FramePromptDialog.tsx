...
export function FramePromptDialog({
    ...
    if (!skipCharacterAuto && !shouldSkipCharacters && characters.length > 0) {
      ...
      if (segmentHasCharacters && segmentCharacterNames.length > 0) {
        ...
        if (matched.length > 0) {
          ...
          // Beat says characters exist but names don't match — use broader text detection
          const segmentText = [
            segment.action,
            segment.subject,
            segment.actionPrompt,
  ...
  // Priority: 1. Pasted prompts, 2. Beat direction keyframe descriptions, 3. Generic builder
  const intelligentPrompt = useMemo(() => {
    ...
    // Priority 2: Beat direction keyframe descriptions (from AI Phase 1)
  ...
  const isEndInvolved = frameType === 'end' || frameType === 'both'
  const promptFieldLabel =
    frameType === 'end'
        ...
        <DialogHeader>
          ...
          {isEndInvolved && (
            <DialogDescription className="text-left text-slate-400">
              End keyframes use a directed edit of the segment&apos;s start frame image so cast,
              wardrobe, and geography stay aligned — avoiding unrelated full renders that confuse video models.
            </DialogDescription>
          )}
        </DialogHeader>
            ...
            <div className="mt-4 p-3 bg-purple-500/10 border border-purple-500/30 rounded-lg flex-shrink-0">
              <div className="flex items-start gap-2">
                ...
                <div className="text-xs text-purple-300">
                  <p className="font-medium mb-1">Content-aware end (edit from start)</p>
                  <p className="text-purple-400/80">
                    The end image is generated by editing the segment&apos;s start frame after this dialogue/action plays — expressions and blocking evolve while scene continuity stays locked.
                  </p>
                </div>
              </div>
            </div>
