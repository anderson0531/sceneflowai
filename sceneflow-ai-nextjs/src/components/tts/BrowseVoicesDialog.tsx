...
import { Search, RefreshCw, Play, Check, Star, X, Volume2 } from 'lucide-react'
import { 
  ...
  const fetchVoices = async () => {
    ...
    try {
      ...
      if (data?.enabled && Array.isArray(data.voices)) {
        ...
        console.warn(`[BrowseVoicesDialog] Provider ${provider} not available:`, data.error)
        setVoices([])
      }
    } catch (err) {
      ...
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-hidden flex flex-col">
        <DialogHeader className="pb-2">
          ...
          >
            ...
            {characterContext ? `Select Voice for ${characterContext.name}` : 'Browse Voices'}
          </DialogTitle>
          <DialogDescription 
          ...
          >
            {characterContext 
              ? 'Choose a voice that matches your character from our library.'
              : 'Browse and select a voice from our library.'
            }
          </DialogDescription>
        </DialogHeader>
