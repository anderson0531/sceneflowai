...
export function InspirationDrawer({ open, onClose, onInsert }: InspirationDrawerProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="fixed right-0 left-auto top-0 bottom-0 translate-x-0 translate-y-0 ml-auto w-[min(100vw,420px)] max-w-full overflow-hidden rounded-none border-l border-gray-800/80 bg-gray-950/95 backdrop-blur-xl pr-[env(safe-area-inset-right)]">
        <DialogHeader className="sticky top-0 z-10 bg-gray-950/90 backdrop-blur-md border-b border-gray-800/50">
          <DialogTitle className="flex items-center gap-2.5 text-gray-100">
        ...
        <div className="flex flex-col h-[calc(100dvh-56px)]">
            ...
            <div className="flex flex-wrap gap-2">
              {QUICK_TAGS.slice(0, 6).map(tag => (
                <button
                  ...
                  disabled={isGenerating}
                  className="text-xs px-3 py-1.5 rounded-lg bg-gray-800/60 border border-gray-700/50 text-gray-300 hover:bg-gray-700/60 hover:border-blue-500/40 hover:text-blue-300 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                >
                  {tag}
                </button>
              ))}
          ...
          {displayItems.length === 0 && (
            <div className="flex-1 flex flex-col items-center justify-center text-center py-16 px-6">
              <div className="w-20 h-20 rounded-2xl bg-gradient-to-br from-blue-500/15 to-indigo-500/15 flex items-center justify-center mb-5 border border-blue-500/20">
                <Lightbulb size={36} className="text-blue-400" />
              </div>
            ...
            <div className="space-y-3">
              ...
              >
                {editingId === variant.id ? (
                  ...
                  // Display Mode - Production-style Card
                  <div 
                    onClick={() => insert(variant)}
                    onDoubleClick={() => startEdit(variant)}
                    className="group relative p-4 rounded-xl border border-gray-700/50 bg-gray-900/50 hover:bg-gray-800/70 hover:border-blue-500/40 cursor-pointer transition-all duration-200 shadow-sm hover:shadow-md hover:shadow-blue-500/5"
                  >
                    ...
                    <div className="flex items-center justify-between text-xs">
                      ...
                      <ArrowRight size={14} className="opacity-0 group-hover:opacity-100 transition-all duration-200 text-blue-400 translate-x-0 group-hover:translate-x-0.5" />
                    </div>
                    ...
                    <div className="absolute right-3 top-3 opacity-0 group-hover:opacity-100 transition-all duration-200">
                      <button
                        ...
                        className={`p-1.5 rounded-lg transition-all duration-200 ${variant.pinned ? 'text-amber-400 bg-amber-500/10' : 'text-gray-500 hover:text-gray-300 hover:bg-gray-700/50'}`}
          ...
          <div className="px-6 sm:px-8 py-4 space-y-2 border-t border-gray-800/60 bg-gradient-to-t from-gray-950 to-gray-950/80">
            <div className="text-xs text-gray-300 font-medium flex items-center gap-1.5">
              <span className="text-amber-400">💡</span> Pro Tips
            </div>
            <ul className="text-[11px] text-gray-500 space-y-1 pl-1">
              <li className="flex items-start gap-2"><span className="text-gray-600">•</span> Click any idea to insert it into your blueprint</li>
              <li className="flex items-start gap-2"><span className="text-gray-600">•</span> Double-click to customize before inserting</li>
              <li className="flex items-start gap-2"><span className="text-gray-600">•</span> Pin your favorites with the pin icon</li>
              <li className="flex items-start gap-2"><span className="text-gray-600">•</span> Generate richer results with detailed keywords</li>
            </ul>
          </div>
        </div>
      </DialogContent>
