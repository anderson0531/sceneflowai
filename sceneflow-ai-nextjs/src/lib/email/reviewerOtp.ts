...
function otpPath(email: string): string {
  return `storyboard-review/otp/${emailHash(email)}.json`
}
...
export async function verifyReviewerEmailOtp(rawEmail: string, code: string): Promise<string> {
  const email = normalizeEmail(rawEmail)
...
export async function consumeReviewerEmailToken(rawEmail: string, token: string): Promise<boolean> {
  const email = normalizeEmail(rawEmail)
  const record = await readOtpRecord(email)
