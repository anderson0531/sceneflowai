...
/**
 * Rules the LLM must follow when populating `beatDirection` on every beat.
 *
 * The block is worded to give downstream prompt builders structured fields
 * they can consume verbatim, avoiding prose reinterpretation of
...
Rules:
• Do NOT invent prop names. keyProps MUST be a subset of scene "Key Props" — never a character name and never a possessive. Do not list built-in architectural hardware (door/hatch/vault/lockdown/valve wheels); put those in the location description.
...
function buildCompactBeatDirectionPromptBlock(includeProps: boolean): string {
  const propFields = includeProps
    ? `
• "keyProps": array of prop labels — SUBSET of the scene's Key Props — visible in THIS beat. Use the exact labels from the scene's Key Props list, never a character name. Omit when no listed prop appears. Do not list door/hatch/vault/lockdown/valve wheels or other built-in architectural hardware.
...
export function buildBeatDirectionSchemaExample(opts?: {
  ...
  if (opts?.compact) {
    const props = opts.includeProps
      ...
      : ''
    return `"beatDirection": {"castInFrame": ["Piper Hayes", "Gideon Croft"], "shotType": "Medium Close-Up", "frozenMoment": "Piper halts Gideon mid-lean, journal pressed to his sternum, her jaw set.", "transition": "CUT"${props}}`
  }
  return `"beatDirection": {
          ...
          "frozenMoment": "Piper halts Gideon mid-lean, journal pressed to his sternum, her jaw set, his eyes wide.",
          "audioCue": "glitching proximity timer chirps twice",
          "transition": "CUT"
        }`
}
