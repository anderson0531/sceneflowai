...
export interface PersistSceneAudioAtomicParams {
  ...
  /**
   * When true, sets visionPhase.scriptUpdatedAt for non-voice writes (sfx/music).
   * Dialogue/narration/description are media, not script edits — they never bump
   * the timestamp, so a queued dialogue PUT is not discarded as stale.
   */
...
const VOICE_AUDIO_TYPES: ReadonlySet<SceneAudioType> = new Set([
  'dialogue',
  'narration',
  'description',
])
...
/**
 * Voice clips must not invalidate a queued script PUT. SFX/music may still
 * bump when the caller opted in.
 */
export function shouldBumpScriptUpdatedAtOnAudioPersist(
  audioType: SceneAudioType,
  updateScriptUpdatedAt?: boolean
): boolean {
  if (!updateScriptUpdatedAt) return false
  if (VOICE_AUDIO_TYPES.has(audioType)) return false
  return true
}
...
/**
 * Apply one scene audio write onto a metadata clone. Used by the locked
 * Sequelize writer and by unit tests that must not open a database.
 */
export function applySceneAudioPersistToMetadata(
  metadata: Record<string, unknown>,
  params: PersistSceneAudioAtomicParams
): PersistSceneAudioResult & { metadata: Record<string, unknown> } {
  ...
  if (shouldBumpScriptUpdatedAtOnAudioPersist(params.audioType, params.updateScriptUpdatedAt)) {
    const visionPhase = (metadata.visionPhase ?? {}) as Record<string, unknown>
