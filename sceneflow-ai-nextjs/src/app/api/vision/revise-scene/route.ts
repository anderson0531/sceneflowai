...
For each recommendation, make the necessary STRUCTURAL or CONTENT changes. Do NOT make superficial rewording that leaves the underlying problem intact.`
  ...
  const preserveInstructions = normalizedPreserve.map((element) => {
    switch (element) {
      ...
      case 'beatDirection':
        return 'Preserve each kept beat\'s existing beatDirection fields verbatim (castInFrame, shotType, cameraAngle, cameraMovement, blocking, emotion, gaze, keyProps, propInteraction, lightingAccent, frozenMoment, audioCue, transition); only add beatDirection for NEW beats'
      case 'beatFrames':
...
- If the recommendation says "condense" the affected beats should carry FEWER or tighter dialogue lines — condensing tightens the writing inside beats, it is not an instruction to cut the scene's beat count
