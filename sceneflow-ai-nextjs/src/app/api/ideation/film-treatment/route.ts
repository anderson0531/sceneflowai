  ...
  const result = {
    ...
    character_descriptions: Array.isArray((parsed as any).character_descriptions)
      ? ((parsed as any).character_descriptions as any[]).map((c: any) => ({
          ...
          coreIdentity: {
            subject: String(c?.coreIdentity?.subject || c?.subject || c?.name || ''),
            ethnicity: String(c?.coreIdentity?.ethnicity || c?.ethnicity || ''),
            keyFeature: String(c?.coreIdentity?.keyFeature || c?.keyFeature || ''),
          },
          appearanceDetails: {
            hairStyle: String(c?.appearanceDetails?.hairStyle || c?.hairStyle || ''),
            hairColor: String(c?.appearanceDetails?.hairColor || c?.hairColor || ''),
            eyeColor: String(c?.appearanceDetails?.eyeColor || c?.eyeColor || ''),
            expression: String(c?.appearanceDetails?.expression || c?.expression || ''),
            build: String(c?.appearanceDetails?.build || c?.build || ''),
          },
