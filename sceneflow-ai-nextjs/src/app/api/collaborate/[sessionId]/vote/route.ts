import { NextRequest, NextResponse } from 'next/server'
import { CollaborationService } from '@/services/CollaborationService'

export async function POST(req: NextRequest, { params }: { params: { sessionId: string } }) {
  const form = await req.formData()
  const ideaId = String(form.get('ideaId') || '')
  const name = String(form.get('name') || 'Guest')
  const rating = Number(form.get('rating') || 0)

  const session = await CollaborationService.getSession(params.sessionId)
  if (!session) return NextResponse.redirect(new URL('/404', req.url))

  // Create ephemeral collaborator for rating
  const collabId = 'guest_' + Math.random().toString(36).slice(2, 9)
  await CollaborationService.submitVote(params.sessionId, collabId, ideaId, Math.min(5, Math.max(1, rating)), `Vote by ${name}`)

  return NextResponse.redirect(new URL(`/collaborate/${params.sessionId}`, req.url))
}
