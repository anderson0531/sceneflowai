...
| 2025-12-14 | Sync Audio Button Feature | **Manual audio sync from script to timeline**. Added self-contained Sync Audio feature in SceneProductionManager: 1) Extracted audio building logic into reusable `buildAudioTracksFromScene()` function that builds AudioTracksData (voiceover, dialogue, music, SFX) from scene data, 2) Added `handleSyncAudio()` callback that rebuilds audio tracks and shows toast with track count, 3) Added "Sync Audio" button with RefreshCw icon in timeline header next to Generate button. **Key architecture decision**: Feature is fully self-contained (no prop threading required) since SceneProductionManager already receives the `scene` prop with all audio data. This avoids the prop threading error ("onSyncAudio is not defined") that caused a previous implementation to be backed out. Files: `SceneProductionManager.tsx`. | ✅ Implemented |
