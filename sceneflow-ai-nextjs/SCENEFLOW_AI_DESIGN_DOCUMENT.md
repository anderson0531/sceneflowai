...
| Beat-first derive segments | `src/app/api/scenes/[sceneId]/derive-segments/route.ts` |
| Beat migration | `src/lib/script/beatMigration.ts` |
| Derive segments from beats | `src/lib/scene/deriveSegmentsFromBeats.ts` |
| Beat video prompts | `src/lib/scene/beatVideoPromptCompiler.ts` |
| Storyboard approval API | `src/app/api/scenes/[sceneId]/approve-storyboard/route.ts` |
...
| **Production Streams** | | |
| Production Streams Panel | `src/components/workflow/ProductionStreamsPanel.tsx` | Animatic/Video stream type tabs, render settings |
| Final Cut Stream Selector | `src/components/workflow/FinalCutStreamSelector.tsx` | Per-scene stream type selection for export |
| Scene Animatic API | `src/app/api/export/scene-animatic/route.ts` | Single-scene animatic render endpoint |
| Feedback Summarize API | `src/app/api/review/summarize/route.ts` | Gemini-powered feedback analysis |
| **Storage Management APIs** | | |
| Storage Usage | `src/app/api/storage/usage/route.ts` | Storage breakdown with tier limits |
| Storage Archive | `src/app/api/storage/archive/route.ts` | Archive to GCS Nearline/Coldline |
| Storage Delete | `src/app/api/storage/delete/route.ts` | Permanent file deletion |
| Storage Restore | `src/app/api/storage/restore/route.ts` | Restore from cold storage |
| Production Streams Storage | `src/app/api/storage/production-streams/route.ts` | List/delete stream files |
...
|------|----------|-----------|--------|
| 2026-05-22 | Beat-First Production Pipeline (v2.48) | **Unified production timeline: ordered `beats[]` → Express storyboard frames → approval gate → deterministic segments → I2V video**. Problem: Storyboard frames (optional, dialogue-only) and Gemini `generate-segments` drifted — action beats reused establishing shots, orphan segments had no storyboard anchor, and style drifted between image and video. **Solution**: 1) **`SceneBeat` model** (`segmentTypes.ts`, `beatMigration.ts`) — interleaved dialogue, action, and narration beats with stable IDs; legacy `dialogue[]` / `narration` dual-written for exports, 2) **Script v2** outputs `beats[]`; projects auto-migrate on load via `migrateProjectToBeats`, 3) **Express** generates one Imagen frame per beat with project `artStyle`; sets `storyboardStatus: pending_review`, 4) **Storyboard Review** (`StoryboardReviewPanel`) — hard gate: `approve-storyboard` required before `derive-segments` and `generate-asset`, 5) **`deriveSegmentsFromBeats`** — 1:1 beat→segment mapping, locked start frames, I2V default, `needsSplit` + Extend for long dialogue (native EXT chain), 6) **`beatVideoPromptCompiler`** — SFX-free Veo prompts with art-style lock. Feature flag: `BEAT_FIRST_PIPELINE` / `NEXT_PUBLIC_BEAT_FIRST_PIPELINE` (default on). Legacy `generate-segments` retained behind flag off. | ✅ Implemented |
...
| 2026-01-02 | Audio Timeline UX Improvements (v2.43) | **Fixed track organization, removed awkward start time controls, and added mute buttons to audio cards**. Problems: 1) Start time controls in audio cards were awkward and unintuitive - timing adjustments should be on timeline, 2) Description and Narration were both displaying on V.O. track but narration was incorrectly placed on music track in the timeline visualization, 3) Music track in timeline was empty - actual music audio wasn't being added, 4) No way to mute individual audio tracks without editing the data. **Solution - 5 Key Fixes**: 1) **Removed Start Time Controls** - Removed the Clock icon, "Start:" label, and number input fields from Description, Narration, and Dialogue audio cards. Duration display simplified to just show seconds. Users will adjust timing via drag-to-reposition on timeline (Phase 2 future feature), 2) **Fixed Voiceover Track** - `AudioTracksData.voiceover` changed from single clip to array (`AudioTrackClip[]`). IIFE now pushes both Description and Narration clips to the voiceover array with distinct IDs ('description' and 'narration'). Both appear on the same V.O. track in timeline, 3) **Fixed Music Track** - Added actual music audio (`scene.musicAudio || scene.music?.url`) to the music track array. Music track no longer incorrectly shows narration. `AudioTracksData.music` changed to array to support multiple music clips, 4) **Updated hasAnyAudio Check** - Now includes `hasMusicAudio` check alongside description, narration, dialogue, and SFX to ensure timeline shows when music is present, 5) **Added Mute Buttons** - New `mutedTracks` state tracks which audio types are muted per scene (`Record<number, { description?: boolean; narration?: boolean; dialogue?: boolean; music?: boolean; sfx?: boolean }>`). Added `toggleTrackMute()` handler. Each audio section now has a mute toggle button: Description (blue), Narration (purple), Dialogue (emerald), Music (purple), SFX (amber). Button shows VolumeX icon with red background when muted, Volume2 icon when unmuted. **AudioTimeline Component Updates**: Interface updated to accept arrays for `voiceover` and `music` tracks. `allClips` useMemo iterates arrays with `.forEach()` instead of checking single clips. `renderTrack` calls pass arrays directly. **Visual Mute Indication**: Muted tracks show red VolumeX icon with `bg-red-500/20` background for clear visual distinction. **Key Files**: `ScriptPanel.tsx` (removed start controls, fixed IIFE, added mute buttons, new state/handler), `AudioTimeline.tsx` (interface and array handling). | ✅ Implemented |
| 2026-01-02 | Audio Timeline Visual Fixes (v2.42) | **Fixed card/timeline color coordination, added SFX to timeline, and removed redundant header play button**. Problems: 1) Card colors (purple/blue/green) didn't match AudioTimeline track bar colors, 2) SFX timeline bars not displayed, 3) Extra play button next to Mark Done was redundant with per-track play buttons. **Solution - 5 Key Fixes**: 1) **Description Card → Blue** - Changed Scene Description card from purple to blue (`bg-blue-50 dark:bg-blue-900/20`, `border-blue-200`, etc.) to match V.O. (voiceover) track in AudioTimeline which uses `bg-blue-500`. Added "(V.O.)" label to header. Updated all buttons, inputs, and borders to blue theme, 2) **Narration Card → Purple** - Changed Scene Narration card from blue to purple (`bg-purple-50 dark:bg-purple-900/20`, etc.) to match Music track in AudioTimeline (which displays narration when description exists). Consistent purple theme for all controls, 3) **Dialogue Card → Emerald** - Changed Scene Dialog container from `green-950`/`green-500` to `emerald-950`/`emerald-500` to exactly match the `bg-emerald-500` dialogue track in AudioTimeline, 4) **SFX Timeline Support** - Added SFX clips to the timeline visualization IIFE. Processes `scene.sfxAudio[]` array, uses `scene.sfx[idx].time` for startTime or distributes evenly, estimates 2s duration per clip. Added `maxSfxEnd` to sceneDuration calculation. Updated `hasAnyAudio` check to include SFX. SFX appears as amber bars (`bg-amber-500`) in timeline, 5) **Removed Header Play Button** - Removed the "Play/Stop Audio Button" (`Volume2`/`Square`) from scene header next to Mark Done. Users can now play audio via individual track play buttons or the AudioTimeline component's transport controls. Reduces UI clutter and avoids confusion about which play button to use. **Color Coordination Summary**: V.O./Description=Blue, Narration/Music=Purple, Dialogue=Emerald, SFX=Amber. This matches AudioTimeline's `renderTrack()` color scheme. **Key Files**: `ScriptPanel.tsx` (color updates, SFX timeline, button removal). | ✅ Implemented |
| 2026-01-02 | Audio Timeline UX Fixes (v2.41) | **Fixed narrow start time inputs and added visual audio timeline**. Problems: 1) Start time input fields were too narrow (`w-14`/`w-16` = 56-64px) to display values like `123.5`, preventing users from editing them properly. 2) No visual timeline to show audio clip alignment. **Solution - 2 Key Fixes**: 1) **Widened Start Time Inputs** - Changed all three input fields from `w-14`/`w-16` to `w-20` (80px) and increased font from `text-[10px]` to `text-xs` for better readability. Description, narration, and dialogue cards all updated consistently. Added proper `px-2` padding, 2) **Audio Timeline Visualization** - Added always-visible `AudioTimeline` component at top of Script tab (Option B: always visible strip). Shows visual representation of all audio tracks: voiceover (description/narration), dialogue (with character labels), positioned according to their `startTime` offsets. Timeline dynamically calculates scene duration from audio data. Shows total duration in header. Uses existing `AudioTimeline.tsx` component from `scene-production/`. Tracks displayed: Description as voiceover (purple), Narration as music track for visualization (blue), Dialogue as dialogue track (green) with character labels. Timeline only renders when scene has audio. **Integration**: Added `AudioTimeline`, `AudioTracksData`, `AudioTrackClip` imports from scene-production. Builds `AudioTracksData` object from scene's audio data, respecting custom `startTime` values from Phase 2 controls. Timeline appears between Director's Summary and Scene Description cards. **Key Files**: `ScriptPanel.tsx` (widened inputs, added timeline, new imports). | ✅ Implemented |
| 2026-01-02 | Audio Timeline Integration - Phase 3 (v2.40) | **Integrated custom audio start times with playback engines (Screening Room + Inline Playback)**. Problem: Phase 2 added UI controls for setting custom `startTime` values on audio segments, but these values were not actually used during playback. Both ScriptPlayer (Screening Room) and ScriptPanel (inline playback) were ignoring the stored startTime offsets. **Solution - Playback Engine Integration**: 1) **ScriptPlayer.tsx Narration Timing** - Modified `calculateAudioTimeline()` to check for custom narration startTime: `scene.narrationAudio?.[selectedLanguage]?.startTime`. When present, uses that value instead of default `NARRATION_DELAY_SECONDS` (2s). Added console logging for custom offset detection, 2) **ScriptPlayer.tsx Dialogue Timing** - Each dialogue entry now checks for `dialogue.startTime`. When present, uses that value instead of the sequential cursor. Fallback maintains original sequential behavior with `DIALOGUE_GAP_SECONDS` (0.3s) between lines. Console logs custom startTime usage per character, 3) **ScriptPanel.tsx Description Timing** - Added custom description startTime support via `config.descriptionOffsetSeconds`. Uses `scene.descriptionAudio?.[selectedLanguage]?.startTime ?? 0`. WebAudioMixer already supported `descriptionOffsetSeconds` in `SceneAudioConfig`, 4) **ScriptPanel.tsx Narration Timing** - Same pattern as ScriptPlayer: checks `scene.narrationAudio?.[selectedLanguage]?.startTime`, falls back to calculated offset (after description + 0.35s) or default (2s), 5) **ScriptPanel.tsx Dialogue Timing** - Each dialogue entry uses `dialogue.startTime` when present, otherwise sequential cursor. Updates cursor position even with custom startTime to maintain proper sequencing for subsequent dialogues. **Cursor Logic Enhancement**: When custom startTime is used, the cursor (`currentTime`) is updated to `Math.max(currentTime, dialogueStart) + duration + gap` to handle mixed scenarios where some dialogues have custom times and others don't. **Backward Compatibility**: All changes use optional chaining and nullish coalescing (`??`) so existing projects without `startTime` values continue to work with default sequential timing. **Debug Logging**: Added `[ScriptPlayer]` and `[ScriptPanel]` prefixed logs for custom offset detection to aid in debugging timing issues. **Key Files**: `ScriptPlayer.tsx` (Screening Room playback), `ScriptPanel.tsx` (inline playback), `webAudioMixer.ts` (already supported `descriptionOffsetSeconds`). | ✅ Implemented |
| 2026-01-01 | Audio Start Time Controls - Phase 2 (v2.39) | **Added per-segment start time offset controls for audio playback timing customization**. Problem: Users had no way to control when audio segments (description, narration, dialogue) start playing relative to the scene start. All audio played sequentially from time 0 with no flexibility for creative timing adjustments. **Solution - Start Time Offset Controls**: 1) **New onUpdateAudioStartTime Prop** - Added `onUpdateAudioStartTime?: (sceneIndex: number, audioType: 'description' | 'narration' | 'dialogue', startTime: number, dialogueIndex?: number) => void` to both ScriptPanelProps and SceneCardProps. Prop chain flows from VisionPage handler through ScriptPanel to SceneCard, 2) **handleUpdateAudioStartTime Handler** (`/src/app/dashboard/workflow/vision/[projectId]/page.tsx` ~85 lines) - Updates audio data's `startTime` field for description, narration, or dialogue (by index). Persists to database via `saveScenesToDatabase()`. No toast on save to avoid interruption during rapid value changes, 3) **Description Card Start Time Control** - When audio exists and section is expanded, shows inline control below text: Clock icon + "Start at:" label + number input (0.1s step) + "seconds" suffix. Styled in purple to match card theme. Value from `scene.descriptionAudio[selectedLanguage].startTime`, 4) **Narration Card Start Time Control** - Same pattern in blue styling below narration text. Value from `scene.narrationAudio[selectedLanguage].startTime`, 5) **Dialogue Card Start Time Control** - Inline with existing duration display (flex row with gap). Shows when audio has duration. Value from `audioEntry.startTime`. Green styling matches dialogue theme. **Data Structure Extension**: Audio objects now support optional `startTime: number` field alongside existing `url`, `duration`, `generatedAt`, `voiceId`. Default is 0 when not set. **UI Design**: Compact inline controls (w-14/w-16 inputs, text-[10px]) to avoid visual clutter. Controls only appear when audio exists and section is expanded. **Use Cases**: 1) Delay narration to let scene visuals establish, 2) Sync dialogue to specific video moments, 3) Create audio overlaps or gaps for dramatic effect, 4) Fine-tune timing for Screening Room playback. **Future Phase 3**: Advanced timeline features including drag-to-reorder, waveform visualization, and audio clip repositioning via drag. **Key Files**: `ScriptPanel.tsx` (UI controls, prop types, destructuring), `page.tsx` (handler implementation, prop passing). | ✅ Implemented |
| 2026-01-01 | Audio Timeline UX Improvement (v2.38) | **Removed static Audio Timeline Preview Strip and added inline duration display to audio segment cards**. Problem: The Audio Timeline Preview Strip (colored bar showing Desc|Narr|Dialog|SFX|Music readiness) added visual clutter without functional value - it was purely decorative and didn't provide timing control, playback, or duration information. Users couldn't tell how long each audio segment was without playing it. **Solution - Phase 1 Cleanup**: 1) **Removed Audio Timeline Preview Strip** - Deleted the static 90-line IIFE block in ScriptPanel.tsx that rendered the colored bar. This reduces visual noise and UI clutter, 2) **Added duration display to Scene Description card** - When audio is ready, the badge now shows duration (e.g., "4.2s") instead of just "Audio Ready". Reads from `scene.descriptionAudio[selectedLanguage].duration`, 3) **Added duration display to Scene Narration card** - Same pattern, shows duration from `scene.narrationAudio[selectedLanguage].duration` in the badge, 4) **Dialogue cards already had duration** - Line 4214 already displayed `audioEntry?.duration.toFixed(1)}s` below each dialogue line. **Sequential Playback**: Audio plays in natural order: Description → Narration → Dialogue → SFX (Music runs alongside). Each card has its own play/pause button for individual preview. **Future Phases**: Phase 2 will add per-segment start time controls. Phase 3 will add advanced timeline features (drag-to-reorder, waveform visualization). **Key Files**: `ScriptPanel.tsx` (removed ~90 lines of Audio Timeline Preview Strip, updated Description and Narration card badges). | ✅ Implemented |
...
| 2026-01-02 | Vision Page Navigation Refactor v3.0 | **Comprehensive refactor of scene navigation for single-scene-at-a-time focus with timeline as primary navigation**. Problems: 1) Dual navigation patterns (accordion list + hidden timeline) caused confusion, 2) Timeline hidden by default (`showTimeline: false`) - primary nav was hidden, 3) Scene list rendered ALL scenes even when one was selected - performance/clutter issues, 4) "Pro Tips" naming didn't convey workflow guidance intent, 5) Accordion expand/collapse logic was complex and error-prone. **Solution - 5 Changes**: 1) **Timeline Always Visible** - Changed `showTimeline` default from `false` to `true` in `ScriptPanel.tsx`. Removed timeline toggle button from header - timeline is now always-on primary navigation. Timeline slot renders unconditionally when available, 2) **Single Scene Card Only** - Modified `displayedScenes` useMemo to ALWAYS return single scene (using `selectedSceneIndex` or defaulting to 0). Removed fallback that showed all scenes. Set `isWorkflowOpen={true}` always since only one scene is visible. Removed unused `openSceneIdx` accordion state, 3) **Prev/Next Scene Navigation** - Added `onPrevScene` and `onNextScene` props to `SceneSelector` component. Added navigation bar below scene cards with "Prev Scene | Scene X of Y | Next Scene" buttons. VisionPage passes handlers that update `selectedSceneIndex`, 4) **Keyboard Navigation** - Added `useEffect` in VisionPage with keydown listener for: Arrow Left/Up = prev scene, Arrow Right/Down = next scene, Number keys 1-9 = jump to scene. Ignores input when user is typing in input/textarea elements, 5) **Renamed Pro Tips to Workflow Guide** - Changed header text from "Pro Tips" to "Workflow Guide" in `ProTipsChecklist.tsx` to better convey its purpose as workflow step guidance. **Deferred to Next Iteration**: Full WorkflowGuidePanel extraction as separate component, SceneCard sub-component extraction for maintainability (5000+ line file). **Wireframe**: Scene Timeline (always visible) at top → Single SceneCard with workflow tabs → Workflow Guide in sidebar. Navigation via: Timeline click, Prev/Next buttons, ← → arrow keys, 1-9 number keys. Files: `ScriptPanel.tsx` (timeline default, single scene, simplified workflow), `SceneSelector.tsx` (prev/next props, nav bar), `page.tsx` (keyboard nav, prev/next handlers), `ProTipsChecklist.tsx` (rename). | ✅ Implemented |
